// BV SHOP 出貨助手 - 內容腳本 (完整版 - 支援物流編號比對與排版調整 + 嘉里大榮)
(function() {
  'use strict';
  ...
  function detectCurrentPage() {
    ...
    // 嘉里大榮物流單頁面
    if (hostname.includes('bvshop-manage.bvshop.tw') && 
        pathname.includes('order_multi_print_ktj_logistics')) {
      console.log('偵測到嘉里大榮物流單頁面');
      return { type: 'ktj', provider: 'ktj' };
    }
    // 7-11 物流單頁面
    else if (hostname.includes('myship.7-11.com.tw') || 
  ...
  async function handleKTJPage() {
    ...
    try {
      ...
      console.error('處理嘉里大榮物流單失敗:', error);
    ...
    if (chrome.storage && chrome.storage.local) {
      chrome.storage.local.get(['bvShippingData'], (result) => {
        if (!result.bvShippingData || result.bvShippingData.length === 0) {
          showNotification('請先至超商物流單頁面或嘉里大榮頁面抓取物流單', 'warning');
          return;
        }
  ...
  function fetchDetailData() {
    ...
    orderContents.forEach((content, index) => {
      ...
      // 提取物流編號 - 支援 8 碼或 12 碼，以及嘉里大榮格式
      let logTraceId = '';
      const logTraceElement = clone.querySelector('.showLogTraceID');
      if (logTraceElement) {
        const text = logTraceElement.textContent;
        // 修改正則：F 或 E 開頭（超商），或 KTJ 開頭（嘉里大榮）
        const match = text.match(/物流編號[:\s]*([FE]\d{7}(?:\d{4})?|KTJ\w+)/);
        if (match) {
          logTraceId = match[1];
        }
      }
  ...
  function saveProviderSettings() {
    ...
    if (chrome.storage && chrome.storage.local) {
      chrome.storage.local.set({
        ...
        showNotification(`已儲存 ${provider === 'default' ? '通用' : provider === 'ktj' ? '嘉里大榮' : provider} 的排版設定`, 'success');
      });
    }
  }
  ...
  function generatePages(printOrder, settings) {
    ...
    shippingData.forEach((data, index) => {
      if (data.serviceCode) {
        ...
        // 如果是 12 碼，也儲存前 8 碼版本
        if (code.length === 12 && (code.startsWith('F') || code.startsWith('E'))) {
      ...
      case 'paired-sequential': // 物流單-出貨明細（正序）
        detailData.forEach((detail, index) => {
          ...
          if (detail.logTraceId) {
            ...
            else if (detailCode.length === 8 && (detailCode.startsWith('F') || detailCode.startsWith('E'))) {
            ...
            else if (detailCode.length === 12 && (detailCode.startsWith('F') || detailCode.startsWith('E'))) {
            ...
            // 4. 嘉里大榮的配對（使用訂單編號）
            else if (detailCode.startsWith('KTJ')) {
              // 對於嘉里大榮，嘗試用訂單編號配對
              for (const [code, data] of shippingMap) {
                if (data.orderNo === detail.orderNo || code.includes(detail.orderNo)) {
                  shipping = data;
                  console.log(`  ✓ 嘉里大榮配對成功: 訂單編號 ${detail.orderNo}`);
                  break;
                }
              }
            }
  ...
  function loadAllProviderSettings() {
    ['default', 'seven', 'family', 'hilife', 'okmart', 'ktj'].forEach(provider => {
      if (chrome.storage && chrome.storage.local) {
        chrome.storage.local.get([`bvProviderSettings_${provider}`], (result) => {
  ...
  function detectProviderFromHTML(html) {
    ...
    if (html.includes('嘉里大榮') || html.includes('KTJ')) return 'ktj';
    return 'default';
  }
  ...
  function showNotification(message, type = 'info') {
    // 對物流單頁面和嘉里大榮頁面，使用簡單的通知方式
    if (currentPage.type === 'shipping' || currentPage.type === 'ktj') {
  ...
  } else if (currentPage.type === 'ktj') {
    // 嘉里大榮頁面處理
    console.log('偵測到嘉里大榮物流單頁面');
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => {
        setTimeout(handleKTJPage, 1000);
      });
    } else {
      setTimeout(handleKTJPage, 1000);
    }
  ...
  window.bvDebug = {
    ...
    checkStorage: function() {
      chrome.storage.local.get(null, (data) => {
        console.log('所有儲存的資料:', data);
      });
    },
    clearKTJ: function() {
      chrome.storage.local.get(['bvShippingData'], (result) => {
        if (result.bvShippingData) {
          const filtered = result.bvShippingData.filter(item => item.provider !== 'ktj');
          chrome.storage.local.set({ bvShippingData: filtered }, () => {
            console.log('已清除嘉里大榮資料');
          });
        }
      });
    }
